...
void NNGridTable::addPoint(const LPoint2D *lp) {
  ...
  int xi = static_cast<int>(lp->pos(0)/csize) + tsize;
  if (xi < 0 || xi > 2*tsize)                      // 対象領域の外
    return;
  int yi = static_cast<int>(lp->pos(1)/csize) + tsize;
...
const LPoint2D *NNGridTable::findClosestPoint(const LPoint2D *clp, const Pose2D &predPose) {
  ...
  int cxi = static_cast<int>(glp.pos(0)/csize) + tsize;
  if (cxi < 0 || cxi > 2*tsize)
    return(nullptr);
  int cyi = static_cast<int>(glp.pos(1)/csize) + tsize;
  ...
  for (int i=-R; i<=R; i++) {
	...
	double d = (lp->pos - glp.pos).squaredNorm();
...
void NNGridTable::makeCellPoints(int nthre, vector<LPoint2D> &ps) {
  ...
  for (size_t i=0; i<table.size(); i++) {
    ...
    if (lps.size() >= nthre) {           // 点数がnthreより多いセルだけ処理する
      ...
      Eigen::Vector2d cog(0,0), avgnorm(0,0);
      int sid=0;
      for (size_t j=0; j<lps.size(); j++) {
        const LPoint2D *lp = lps[j];
	cog    += lp->pos;            // 位置を累積
	avgnorm+= lp->norm;      // 法線ベクトル成分を累積
...
//        printf("sid=%d\n", lp->sid);
      ...
      cog     /= lps.size();          // 平均
      avgnorm /= avgnorm.norm();      // 平均（正規化）
      sid /= lps.size();                 // スキャン番号の平均とる場合

      LPoint2D newLp(sid, cog);       // セルの代表点を生成
      newLp.setNormal(avgnorm);           // 法線ベクトル設定
